...
export async function POST(req: Request) {
  ...
  if (!code || code.length < 12 || code.length > 64) {
  ...
  try {
    const result = await db.transaction(async (tx) => {
      ...
      if (!redemption || redemption.packageStatus !== "active") {
      ...
      const existing = await tx
        ...
        .where(
          and(
            ...
            eq(subscriptions.packageId, redemption.packageId),
      ...
      const [subscription] = await tx
        ...
        .values({
          ...
          providerReference: `redeem:${redemption.id}`,
        })
      ...
      return {
        ...
        packageSlug: redemption.packageSlug,
    ...
    return NextResponse.json({
      ok: true,
      message: `${result.packageName} berhasil diaktifkan.`,
      package: result.packageSlug,
