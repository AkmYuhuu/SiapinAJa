...
export default function TermsPage() {
  return <Suspense fallback={null}><TermsContent /></Suspense>;
}
...
function TermsContent() {
  ...
  return (
    <div className="min-h-screen bg-bg text-ink">
      <header className="border-b border-border bg-surface"><div className="mx-auto flex h-16 max-w-4xl items-center justify-between px-4"><Link href={returnTo} className="text-sm font-semibold text-ink-secondary hover:text-ink">← Kembali</Link><Link href="/" className="text-base font-bold tracking-tight text-ink">Siapin<span className="text-accent-strong">Aja</span></Link><Link href={privacyHref} className="text-sm font-medium text-accent-strong hover:underline">Privasi</Link></div></header>
      <main className="mx-auto max-w-3xl px-4 py-10 lg:py-14"><p className="text-xs font-semibold uppercase tracking-wider text-accent-strong">Dokumen Legal</p><h1 className="mt-2 text-3xl font-bold tracking-tight">Syarat & Ketentuan</h1><p className="mt-2 text-sm text-ink-faint">Berlaku mulai 16 Agustus 2026</p><div className="mt-8 space-y-8 text-sm leading-7 text-ink-secondary">
