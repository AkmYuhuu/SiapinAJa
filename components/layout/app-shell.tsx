...
export function AppShell({ children }: { children: ReactNode }) {
  ...
  return (
    <div className="min-h-screen">
      <aside className="app-sidebar fixed inset-y-0 left-0 z-30 hidden w-60 border-r border-border bg-surface lg:block"><SidebarContent /></aside>
      {mobileOpen && <div className="fixed inset-0 z-40 lg:hidden"><div className="absolute inset-0 bg-black/30" onClick={() => setMobileOpen(false)} /><aside className="app-sidebar absolute inset-y-0 left-0 w-64 bg-surface shadow-[8px_0_32px_rgba(43,40,35,0.15)]"><SidebarContent onNavigate={() => setMobileOpen(false)} /></aside></div>}
      ...
      <div className="app-shell-content lg:pl-60">
        <header className="app-topbar print-hide sticky top-0 z-20 flex h-14 items-center gap-3 border-b border-border bg-bg/90 px-4 backdrop-blur lg:px-6">
          <button className="flex size-8 items-center justify-center rounded-md text-ink-secondary hover:bg-surface-muted lg:hidden" onClick={() => setMobileOpen(true)} aria-label="Buka menu"><svg className="size-5" viewBox="0 0 20 20" fill="none" aria-hidden><path d="M3 5.5h14M3 10h14M3 14.5h14" stroke="currentColor" strokeWidth="1.6" strokeLinecap="round" /></svg></button>
          ...
          <form ref={formRef} onSubmit={goDashboard} className="relative ml-auto hidden md:block">
            ...
            {open && q.trim() && results && (results.toolHits.length > 0 || results.projectHits.length > 0) && <div className="absolute left-0 top-full z-30 mt-1.5 w-96 max-h-[26rem] overflow-y-auto rounded-lg border border-border bg-surface p-1.5 shadow-[0_12px_32px_rgba(43,40,35,0.18)]">
          ...
          <div className="ml-auto flex items-center gap-1 md:ml-2"><a href="/help" className="hidden size-8 items-center justify-center rounded-md text-ink-secondary hover:bg-surface-muted sm:flex" title="Bantuan" aria-label="Bantuan"><Icon name="help" className="size-4" /></a><button onClick={() => router.push("/history")} className="flex size-8 items-center justify-center rounded-md text-ink-secondary hover:bg-surface-muted" title="Riwayat" aria-label="Riwayat"><Icon name="history" className="size-4" /></button><button onClick={() => router.push("/account")} className="flex size-8 items-center justify-center rounded-full bg-accent-soft font-bold text-accent-ink" title="Akun" aria-label="Akun">{(session?.name ?? "?").slice(0, 1).toUpperCase()}</button></div>
        </header>
