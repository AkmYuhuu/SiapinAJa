...
export function SupportWidget() {
  ...
  useEffect(() => {
    ...
    userChannel.on("broadcast", { event: "message" }, ({ payload }) => {
      ...
      const target = conversations.find((item) => item.id === incomingConversationId);
      const targetType = target?.type ?? (incomingConversationId === earlyConversationId ? "early_access" : "support");
      if (incomingConversationId === conversationIdRef.current) {
        setMessages((current) => mergeMessage(current, incoming));
        setAdminTyping(false);
      }
      showNotice(
        targetType === "early_access" ? "Ada pembaruan Early Access" : "Ada balasan baru dari admin",
