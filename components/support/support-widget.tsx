...
export function SupportWidget() {
  ...
  useEffect(() => {
    ...
    const startFallback = () => {
      ...
      fallbackPollTimer.current = setInterval(() => { void loadConversation(activeConversationId, true); }, 1500);
    };
    ...
    channel
      .on("broadcast", { event: "typing" }, ({ payload }) => {
        ...
        if (typing) typingHideTimer.current = setTimeout(() => setAdminTyping(false), 1800);
      })
  ...
  async function loadConversation(id: string, silent = false) {
    ...
    if (silent) {
      setMessages((current) => (Array.isArray(data.messages) ? data.messages.reduce((acc: Message[], item: Message) => mergeMessage(acc, item), current) : current));
    } else {
  ...
  async function sendMessage(event: React.FormEvent) {
    ...
    if (!response.ok) return;
    setMessage("");
    const id = data.conversationId ?? conversationId;
