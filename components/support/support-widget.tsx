...
export function SupportWidget() {
  ...
  const publishTyping = (isTyping: boolean) => {
    ...
    void channelRef.current.send({ type: "broadcast", event: "typing", payload: { senderType: "user", isTyping } });
  };
  ...
  useEffect(() => {
    ...
    channel
      .on("broadcast", { event: "typing" }, ({ payload }) => {
        ...
        if (payload?.isTyping) window.setTimeout(() => setAdminTyping(false), 1800);
      })
      .on("postgres_changes", { event: "INSERT", schema: "public", table: "support_messages", filter: `conversation_id=eq.${conversationId}` }, (payload) => {
        ...
        if (row.sender_type === "admin") setAdminTyping(false);
      })
      .on("postgres_changes", { event: "UPDATE", schema: "public", table: "support_conversations", filter: `id=eq.${conversationId}` }, (payload) => {
  ...
  const currentConversation = useMemo(() => conversations.find((conversation) => conversation.id === conversationId), [conversations, conversationId]);
  ...
  function resetPanel() { setView("menu"); setMessage(""); setConversationId(""); setMessages([]); setAdminTyping(false); }
  ...
  return (
    <>
      {open && <div className="fixed bottom-20 right-4 z-50 w-[min(92vw,390px)] overflow-hidden rounded-xl border border-border bg-surface shadow-[0_18px_50px_rgba(43,40,35,0.2)]">
        ...
        {view === "early" && <div className="max-h-[70vh] overflow-y-auto p-4">
        ...
        {view === "chat" && <div className="flex h-[520px] flex-col">
          ...
          <div className="min-h-0 flex-1 space-y-3 overflow-y-auto p-4">
            ...
            {adminTyping && <TypingDots label="Admin sedang mengetik" />}
          </div>
