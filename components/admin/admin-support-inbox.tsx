...
export default function AdminSupportInbox() {
  ...
  useEffect(() => {
    ...
    channel
      ...
      .on("postgres_changes", { event: "INSERT", schema: "public", table: "support_messages" }, (payload) => {
        const row = payload.new as { id: string; conversation_id: string; sender_type: string };
        if (row.sender_type !== "user") return;
        if (row.conversation_id === selectedRef.current) {
          void openConversation(row.conversation_id, true);
        } else {
          showNotice("Ada chat terbaru");
          void load();
        }
  ...
  return <>
    ...
    <div className="grid min-h-0 gap-4 lg:h-[calc(100vh-15rem)] lg:grid-cols-[320px_minmax(0,1fr)]">
      <section className="min-h-0 overflow-hidden rounded-lg border border-border bg-surface lg:h-full">
        ...
        <div className="min-h-0 overflow-y-auto lg:h-[calc(100%-77px)]">{loading ? <p className="p-4 text-sm text-ink-secondary">Memuat…</p> : conversations.length === 0 ? <p className="p-4 text-sm text-ink-secondary">Belum ada percakapan terbuka.</p> : <div className="divide-y divide-border">{conversations.map((item) => { const unread = isUnread(item); return <button key={item.id} type="button" onClick={() => toggleConversation(item.id)} className={`w-full px-4 py-3 text-left hover:bg-surface-muted ${selected === item.id ? "bg-surface-muted" : ""}`}><div className="flex items-center justify-between gap-2"><span className={`truncate text-sm ${unread ? "font-extrabold text-ink" : "font-semibold text-ink"}`}>{item.userName}</span><span className="flex items-center gap-1 text-[10px] uppercase text-ink-faint">{unread && <span className="size-2 rounded-full bg-danger" />}{item.type === "early_access" ? "Early" : "Support"}</span></div><p className="mt-1 truncate text-xs text-ink-secondary">{item.subject ?? "Bantuan SiapinAja"}</p><p className={`mt-1 text-[10px] ${unread ? "font-semibold text-danger" : "text-ink-faint"}`}>{unread ? "Pesan baru" : item.status === "needs_info" ? "Menunggu informasi" : item.status}</p></button>; })}</div>}</div>
      </section>
      <section className="min-h-0 overflow-hidden rounded-lg border border-border bg-surface lg:h-full">
